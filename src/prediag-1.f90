...
! Get lowest diagonals
    intguess = 0d0
    call lowdiags( diagonals, cidim, initguessdim, intguess )
...
! Set DSYEVR variabls
    ...
    lwork = 26*initguessdim+10
    liwork= 11*initguessdim
    allocate( isuppz(initguessdim) )
    allocate( eigvectors(initguessdim, initguessdim) )
    allocate(  work( lwork) )
    allocate( iwork(liwork) )
    allocate( eigvalues(initdiagsguess) )
    call dsyevr( jobz, rnge, uplo, initguessdim, subham, initguessdim, vl, &
                vu, il, iu, abstol, eigfound, eigvalues, eigvectors, initguessdim,&
