...
/*
 * compute_diagonals: compute the elements <i|H|i>.
 */
void compute_diagonals(struct det *dlist, int ndets, double *moints1,
		       ...
		       int ncd, double *dgls)
{
    ...
    for (i = 0; i < ncd; i++) {
	dgls[i] = 0.0 + hmatels_cas(dlist[i],dlist[i],moints1,moints2,
				    aelec,belec,nactv);
    }
    for (i = ncd; i < ndets; i++) {
	dgls[i] = 0.0 + hmatels(dlist[i],dlist[i],moints1,moints2,
				aelec, belec, nactv);
    }
    return;
}
...
void compute_hv(struct det *dlist, int ndets, double *moints1, double *moints2,
		...
		int ninto, int ncd, int n1v, int n2v, struct v2row *v2valid,
		struct v2row *v1valid, struct v2row *cvvalid)
{
#ifdef _OPENMP
    int nthreads, tid, cas_chunk, v1_chunk, v2_chunk;
#endif
    int i, j;
    double valij;
    ...
    /*
     * We have partitioned the hamiltonian into sections:
     *  1) CAS-block
     *  2) CAS on 1v block
     *  3) CAS on 2v block
     *  4) 1v-block
     *  5) 1v on 2v block
     *  6) 2v-block
     * First, we compute the CAS-flagged block of H
     * Then we compute other elements of upper triangle.
     */
    for (i = 0; i < ncd; i++) {
	/* CAS-block */ /***** COULD BE HOTSPOT ******/

	c[i] = c[i] + hmatels_cas(dlist[i], dlist[i], moints1, moints2,
				  aelec, belec, ninto) * v[i];
	for (j = i + 1; j < ncd; j++) {

	    valij = hmatels_cas(dlist[i], dlist[j], moints1, moints2,
				aelec, belec, ninto);
 	    c[i] = c[i] + valij * v[j];
	    c[j] = c[j] + valij * v[i];
	}
    ...
    cas_chunk = ncd / nthreads;
    v1_chunk = n1v / nthreads;
    v2_chunk = n2v / nthreads;
#endif
    
#pragma omp parallel \
    shared(ncd,c,v,cvvalid,moints1,moints2,dlist,ninto,aelec,belec,cas_chunk) \
    private(csec, i, j, valij)
    {
#pragma omp for schedule(dynamic, cas_chunk)
    for (i = 0; i < ncd; i++) {

	csec = cvvalid[i].sec;
	while (csec != NULL) {
	    for (j = (csec->first + ncd); j <= (csec->last + ncd); j++) {
...
#pragma omp parallel \
    shared(ncd,c,v,v1valid,moints1,moints2,dlist,ninto,aelec,belec,v1_chunk)\
    private(csec,i,j,valij)
    {
#pragma omp for schedule(dynamic, v1_chunk)
    /* 1v block and 1v.2v block */
    for (i = ncd; i < (ncd + n1v); i++) {
	csec = v1valid[i - (ncd)].sec;
	while (csec != NULL) {
    ...
    /* 2v block */
#pragma omp parallel \
    shared(ncd,n1v,ndets,c,v,v2valid,moints1,moints2,dlist,ninto,aelec,belec,v2_chunk) \
    private(csec, i, j, valij)
    {
#pragma omp for schedule(dynamic, v2_chunk)
    for (i = (ncd + n1v); i < ndets; i++) {
	csec = v2valid[i - (ncd + n1v)].sec;
	while (csec != NULL) {
	    for (j = (csec->first + i); j <= (csec->last + i); j++) {
		valij = hmatels_nocv(dlist[i], dlist[j], moints1, moints2,
				     aelec, belec, ninto);
		c[i] = c[i] + valij * v[j];
		c[j] = c[j] + valij * v[i];
	    }
	       ...
	       double *moints2, int aelec, int belec, int nactv)
{
	...
	/*int numaxc, numbxc, numaxv, 
	  numbxv, numaxcv, numbxcv, detdiff;
...
/* 
 * hmatels_cas: compute matrix element <i|H|j> inside CAS-flag block
 * -----------------------------------------------------------------
 * Input:
 *  deti = determinant i
 *  detj = determinant j
 ...
 *  val = <i|H|j> 
 */
double hmatels_cas(struct det deti, struct det detj, double *moints1,
	       double *moints2, int aelec, int belec, int nactv)
{
    double val = 0.0;
...
/* 
 * hmatels_nocv: compute matrix element <i|H|j> with equal number of
 * virtual orbitals
 * ---------------------------------------
 * Input:
 *  deti = determinant i
 ...
 *  val = <i|H|j> 
 */
double hmatels_nocv(struct det deti, struct det detj, double *moints1,
		    double *moints2, int aelec, int belec, int nactv)
{
    double val = 0.0;
    ...
    detdiff = comparedets_ncas_nocas2virt(
	deti, detj, &numaxc, &numbxc, &numaxv, &numbxv,
	&numaxcv, &numbxcv, &axi, &axf, &bxi, &bxf, nactv);
    if (detdiff > 2) return val;
    val = evaluate_dets_ncas(
...
/* 
 * hmatels_nocv_dgls: compute matrix element <i|H|i>
 * ---------------------------------------
 * Input:
 *  deti = determinant i
 *  detj = determinant j
 ...
 *  val = <i|H|i> 
 */
double hmatels_nocv_dgls(struct det deti, struct det detj, double *moints1,
		    double *moints2, int aelec, int belec, int nactv)
{
    double val = 0.0;
    ...
    int numaxc = 0, numbxc = 0, numaxv = 0, 
	numbxv = 0, numaxcv = 0, numbxcv = 0, detdiff = 0;
    long long int axi = 0x0, axf = 0x0, bxi = 0x0, bxf = 0x0;
