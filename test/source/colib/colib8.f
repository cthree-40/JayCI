...
c
c  version 2 : june,2 2010
c  info(6) =ifmt 
c  10bit compression for nbft>255 
c
c
...
c
      integer nbig, chunk, nword, ibvfac, space,ifac
c
...
c
      integer    nbftmx
      parameter( nbftmx=2**10-1 )
      integer    lrecmn,     lrecmx,         lrcinc,      ndeflt
      parameter( lrecmn=4096, lrecmx=2**16,   lrcinc=4096,  ndeflt=1 )
c
...
c        # use standard 10-bit orbital label packing.
c        # 2 labels:   values= multiple of 16
c                      lenrec=  n*16+n*5 +1  
c                      nval  = 16*n
c        # 4 labels:   values= multiple of 8
c                      lenrec=  n*8+n*5+1
c                      nval  = 8*n
         ifmt=1
      endif
...
c     # loop backwards from the upper bound until a valid nmax is found.
      do 10 nmax = nbig, 0, -chunk
         space = nword + nmax +  (ifac*nmax/chunk)  +
     &    ibvfac * ( ( nmax + 63 ) / 64 )
...
c
       if (ifmt.ne.info(6)) then
        call bummer(' sifd2: ifmt.ne.info(6)',0,2)
       endif
...
c           # disabled
            ierr= -3
            return
c           call ulab32( buffer(lab1), labels, nuw )
         elseif ( nipv.eq.4 ) then
c           # 4 integers/value output.
            nuw=4*num
            call ulab10( buffer(lab1), labels, nuw )
...
c           # 1 integer/value input.
c           nuw=num
c           call plab32( buffer(lab1), labels, nuw )
c           # disabled
             ierr=-1
             return
         elseif ( nipv.eq.2 ) then
c           # 2 integers/value input.
            nuw=2*num
            call plab10( buffer(lab1), labels, nuw )
         else
            call bummer('sife1: nipv=',nipv,wrnerr)
            ierr = -1
            return
...
c
     ...
     & itypea, itypeb, ibvtyp, nrec,   ierr
      integer  info(*),        labels(*),      ibitv(*)
...
c
      do 10 i = 1, num
         write(nlist,fstring(ifmtv)) values(i), (labels(j,i), j=1,nipv)
10    continue
c
      if ( ibvtyp .ne. 0 ) then
...
c
     ...
     &     ibitv, ierr )
      ...
      do i = 1, num
         print *, "x", labels((i-1)*4+1), labels((i-1)*4+2)
      end do
c
      return
      end
...
c
      ...
      parameter(vrsion=2, ntitmx=20, ninchk=5)
...
c      write(*,"(1x,a,i10)") "verin  = ", verin
c      write(*,"(1x,a,i10)") "ntitle = ", ntitle
c      write(*,"(1x,a,i10)") "nsym   = ", nsym
c      write(*,"(1x,a,i10)") "nbft   = ", nbft
c      write(*,"(1x,a,i10)") "ninfo  = ", ninfo
c      write(*,"(1x,a,i10)") "nenrgy = ", nenrgy
c      write(*,"(1x,a,i10)") "nmap   = ", nmap
c     
      if ( ierr.ne.0 ) then
         return
c clm: removing. 08-01-2015
c     elseif ( verin.ne.vrsion ) then
c         call bummer('sifrh1: (verin-vrsion)=',(verin-vrsion),wrnerr)
c         ierr = -2
c         return
...
c
      ...
      integer  info(*)
c
      integer fsplit, last,   num,    ibvtyp,    itypeb, itypea
      integer idummy(1)
...
c
      integer  aoints, lstflg, itypea, itypeb, 
...
c
         call sifcount(ntot,nsym,nbpsy,array,small,itypea,kntin,symoff)
      if ( itypea.eq.0 ) then
c
...
c
      ...
      subroutine sifcount(ntot,nsym,nbpsy,array,small,itypea,
     .                    kntin,symoff)
      implicit none
      integer ntot,nsym,nbpsy(*),itypea,kntin(*),symoff(*)
      real*8  array(*),small
      integer skipd,j2,jsym,j1,ij0,i2,isym,i1,ij,ijsym,i,j
c
      integer nndxf
      nndxf(i) = ( i * (i-1) ) / 2
c
...
c
               do 120 i = i1, i2
                  do 110 j = i1, i
                     ij = ij + 1
                     if ( abs(array(ij)).gt.small) ntot=ntot+1 
110               continue
120            continue
            endif
130      continue
...
260      continue
        ...
        call bummer('sifcount: invalid itypea=',itypea,2)
       endif
         return
         end

        subroutine ulab10( buffer, label, nuw )
        implicit none
        integer*8 buffer(*) 
        integer*8 label(*),nuw,nul
        integer*8  tmp,ioff,joff,i
        integer*8 m10,m8,m6,m4,m2
        parameter (m10=2**10-1,m8=2**8-1, m6=2**6-1,m4=2**4-1,m2=2**2-1)
*@ifndef int64
*          call bummer('ulab10 unsupported for 32 bit',0,2)
*@endif
c
c       operate on groups of  32 labels
c       we assume that the labels field is a multiple of 32
c       while nuw may not be a multiple of 32, i.e.
c       labels(1:((nuw-1)/32+1)*32 ) 
c
c       initialize the non-defined labels in the last group 
c       of 32 labels to zero.

        nul=((nuw-1)/32+1)*32
        ioff=0
        joff=0
         do i=1,nul/32
            label(ioff+1)=iand(m10,ishft(buffer(joff+1),-54))
            label(ioff+2)=iand(m10,ishft(buffer(joff+1),-44))
            label(ioff+3)=iand(m10,ishft(buffer(joff+1),-34))
            label(ioff+4)=iand(m10,ishft(buffer(joff+1),-24))
            label(ioff+5)=iand(m10,ishft(buffer(joff+1),-14))
            label(ioff+6)=iand(m10,ishft(buffer(joff+1),-4))
            tmp  = iand(m4,buffer(joff+1))
            label(ioff+7)=ior(iand(m6,ishft(buffer(joff+2),-58)),
     .                    ishft(tmp,6))
            label(ioff+8)=iand(m10,ishft(buffer(joff+2),-48))
            label(ioff+9)=iand(m10,ishft(buffer(joff+2),-38))
            label(ioff+10)=iand(m10,ishft(buffer(joff+2),-28))
            label(ioff+11)=iand(m10,ishft(buffer(joff+2),-18))
            label(ioff+12)=iand(m10,ishft(buffer(joff+2),-8))
            tmp  = iand(m8,buffer(joff+2))
            label(ioff+13)=ior(iand(m2,ishft(buffer(joff+3),-62)),
     .                    ishft(tmp,2))
            label(ioff+14)=iand(m10,ishft(buffer(joff+3),-52))
            label(ioff+15)=iand(m10,ishft(buffer(joff+3),-42))
            label(ioff+16)=iand(m10,ishft(buffer(joff+3),-32))
            label(ioff+17)=iand(m10,ishft(buffer(joff+3),-22))
            label(ioff+18)=iand(m10,ishft(buffer(joff+3),-12))
            label(ioff+19)=iand(m10,ishft(buffer(joff+3),-2))
            tmp  = iand(m2,buffer(joff+3))
            label(ioff+20)=ior(iand(m8,ishft(buffer(joff+4),-56)),
     .                    ishft(tmp,8))
            label(ioff+21)=iand(m10,ishft(buffer(joff+4),-46))
            label(ioff+22)=iand(m10,ishft(buffer(joff+4),-36))
            label(ioff+23)=iand(m10,ishft(buffer(joff+4),-26))
            label(ioff+24)=iand(m10,ishft(buffer(joff+4),-16))
            label(ioff+25)=iand(m10,ishft(buffer(joff+4),-6))
            tmp  = iand(m6,buffer(joff+4))
            label(ioff+26)=ior(iand(m4,ishft(buffer(joff+5),-60)),
     .                    ishft(tmp,4))
            label(ioff+27)=iand(m10,ishft(buffer(joff+5),-50))
            label(ioff+28)=iand(m10,ishft(buffer(joff+5),-40))
            label(ioff+29)=iand(m10,ishft(buffer(joff+5),-30))
            label(ioff+30)=iand(m10,ishft(buffer(joff+5),-20))
            label(ioff+31)=iand(m10,ishft(buffer(joff+5),-10))
            label(ioff+32)=iand(m10,buffer(joff+5))
            ioff=ioff+32
            joff=joff+5
         enddo
c       initialize the non-defined labels in the last group 
c       of 32 labels to zero.
        ...
        subroutine plab10( buffer, label, nuw )
        implicit none
        integer*8 buffer(*)
        integer*8 label(*),nuw,nul
        integer*8  tmp,ioff,joff,i
        ...
        parameter (m10=2**10-1,m8=2**8-1, m6=2**6-1,m4=2**4-1,m2=2**2-1)
*@ifndef int64
*          call bummer('plab10 unsupported for 32 bit',0,2)
*@endif
c
c       operate on groups of  32 labels
c       we assume that the labels field is a multiple of 32
...
c       of 32 labels to zero.
        ...
        joff=0
         do i=1,nul/32
            tmp=ishft(iand(m10,label(ioff+1)),54)
            tmp=ior(tmp,ishft(iand(m10,label(ioff+2)),44))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+3)),34))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+4)),24))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+5)),14))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+6)),4))
            tmp=ior(tmp,iand(m4,ishft(label(ioff+7),-6)))
            buffer(joff+1)=tmp
            tmp=ishft(iand(m6,label(ioff+7)),58)
            tmp=ior(tmp,ishft(iand(m10,label(ioff+8)),48))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+9)),38))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+10)),28))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+11)),18))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+12)),8))
            tmp=ior(tmp,iand(m8,ishft(label(ioff+13),-2)))
            buffer(joff+2)=tmp
            tmp=ishft(iand(m2,label(ioff+13)),62)
            tmp=ior(tmp,ishft(iand(m10,label(ioff+14)),52))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+15)),42))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+16)),32))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+17)),22))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+18)),12))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+19)),2))
            tmp=ior(tmp,iand(m2,ishft(label(ioff+20),-8)))
            buffer(joff+3)=tmp
            tmp=ishft(iand(m8,label(ioff+20)),56)
            tmp=ior(tmp,ishft(iand(m10,label(ioff+21)),46))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+22)),36))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+23)),26))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+24)),16))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+25)),6))
            tmp=ior(tmp,iand(m6,ishft(label(ioff+26),-4)))
            buffer(joff+4)=tmp
            tmp=ishft(iand(m4,label(ioff+26)),60)
            tmp=ior(tmp,ishft(iand(m10,label(ioff+27)),50))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+28)),40))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+29)),30))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+30)),20))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+31)),10))
            tmp=ior(tmp,ishft(iand(m10,label(ioff+32)),0))
            buffer(joff+5)=tmp
            ioff=ioff+32
            joff=joff+5
         enddo
        ...
        subroutine plab40( buffer, label, nuw )
        implicit none
        integer*8 buffer(*)
        integer*8 label(*),nuw,nul
        integer*8  tmp,ioff,joff,i
        integer*8  m40,m24,m16,m8,m32 
        parameter (m40=2**40-1,m24=2**24-1,m16=2**16-1,m8=2**8-1)
        parameter (m32=2**32-1)
*@ifndef int64
*          call bummer('plab40 unsupported for 32 bit',0,2)
*@endif
c
c       operate on groups of  8 labels
c       we assume that the labels field is a multiple of 8
c       while nuw may not be a multiple of 8, i.e.
c       labels(1:((nuw-1)/8+1)*8 ) 
c
c       initialize the non-defined labels in the last group 
c       of 8 labels to zero.

        nul=((nuw-1)/8+1)*8
        if (mod(nuw,8).ne.0) then
          do i=nuw+1,nul
            label(i)=0
          enddo
        endif
        ...
        joff=0
         do i=1,nul/8
            tmp=ishft(iand(m40,label(ioff+1)),24)
            tmp=ior(tmp,iand(m24,ishft(label(ioff+2),-16)))
            buffer(joff+1)=tmp
            tmp=ishft(iand(m16,label(ioff+2)),48)
            tmp=ior(tmp,ishft(iand(m40,label(ioff+3)),8))
            tmp=ior(tmp,iand(m8,ishft(label(ioff+4),-32)))
            buffer(joff+2)=tmp
            tmp=ishft(iand(m32,label(ioff+4)),32)
            tmp=ior(tmp,iand(m32,ishft(label(ioff+5),-8)))
            buffer(joff+3)=tmp
            tmp=ishft(iand(m8,label(ioff+5)),56)
            tmp=ior(tmp,ishft(iand(m40,label(ioff+6)),16))
            tmp=ior(tmp,iand(m16,ishft(label(ioff+7),-24)))
            buffer(joff+4)=tmp
            tmp=ishft(iand(m24,label(ioff+7)),40)
            tmp=ior(tmp,ishft(iand(m40,label(ioff+8)),0))
            buffer(joff+5)=tmp
            ioff=ioff+8
            joff=joff+5
         enddo
        ...
        subroutine ulab40( buffer, label, nuw )
        implicit none
        integer*8 buffer(*)
        integer*8 label(*),nuw,nul
        integer*8  tmp,ioff,joff,i
        ...
        parameter (m32=2**32-1)
*@ifndef int64
*          call bummer('ulab40 unsupported for 32 bit',0,2)
*@endif
c
c       operate on groups of  8 labels
c       we assume that the labels field is a multiple of 8
...
c       of 8 labels to zero.
        ...
        joff=0
         do i=1,nul/8
            label(ioff+1)=iand(m40,ishft(buffer(joff+1),-24))
            tmp  = iand(m24,buffer(joff+1))
            label(ioff+2)=ior(iand(m16,ishft(buffer(joff+2),-48)),
     .                    ishft(tmp,16))
            label(ioff+3)=iand(m40,ishft(buffer(joff+2),-8))
            tmp  = iand(m8,buffer(joff+2))
            label(ioff+4)=ior(iand(m32,ishft(buffer(joff+3),-32)),
     .                    ishft(tmp,32))
            tmp  = iand(m32,buffer(joff+3))
            label(ioff+5)=ior(iand(m8,ishft(buffer(joff+4),-56)),
     .                    ishft(tmp,8))
            label(ioff+6)=iand(m40,ishft(buffer(joff+4),-16))
            tmp  = iand(m16,buffer(joff+4))
            label(ioff+7)=ior(iand(m24,ishft(buffer(joff+5),-40)),
     .                    ishft(tmp,24))
            label(ioff+8)=iand(m40,buffer(joff+5))
            ioff=ioff+8
            joff=joff+5
         enddo
c       initialize the non-defined labels in the last group 
