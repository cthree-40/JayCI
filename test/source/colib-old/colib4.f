...
C
C Revision 1.1  2000/03/23 10:56:11  columbus
C Initial revision
C
C Revision 1.2.1.1  1998/02/04 10:17:48  calderon
C nrowmx=255, nwlkmx=2**20-1
C
C Revision 1.2  1998/02/04 09:14:48  calderon
C packing
C
C Revision 1.1  1998/01/14 12:13:09  calderon
C Initial revision
C
C Revision 5.2.1.1  1997/07/02 11:50:05  calderon
C niko_version1
C
C Revision 5.2  1997/06/16 11:13:11  calderon
C COLUMBUS52_vudhi
C
C Revision 5.0  1996/06/26 12:26:08  calderon
C  Columbus 5.0
C
C Revision 1.1  1996/04/23 15:24:14  calderon
C Initial revision
C
ccolib part=4 of 9. general vector and math routines.
...
c
            call dscal_wr( nur, tnorm, u(1,k), 1 )
c
...
c
            do 120 j = (k+1), nuc
               term = -tnorm * ddot_wr( nur, u(1,j), 1, su, 1 )
               if ( term .ne. zero )
     &          call daxpy_wr( nur, term, u(1,k), 1, u(1,j), 1 )
               tfact = tfact + term**2
120         continue
140      continue
c
...
c
            do 220 j = (k+1), nuc
               term = -ddot_wr( nur, u(1,j), 1, u(1,k), 1 )
               if ( term .ne. zero )
     &          call daxpy_wr( nur, term, u(1,k), 1, u(1,j), 1 )
               tfact = tfact + term**2
220         continue
...
c
      ...
      lm = 2
      do 100 m = 2, n
         c(m) = ddot_wr( m, a(lm),1, b,1 )
         if ( b(m) .ne. zero ) call daxpy_wr( (m-1), b(m), a(lm),1, 
     + c,1 )
