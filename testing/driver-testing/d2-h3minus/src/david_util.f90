...
contains
  ...
  subroutine dvd_initial( vectors1, num_vecs, dim_vecs, moints1, moints1len, &
    ...
    integer :: i, j
#ifdef DEBUGHV
    real*8, dimension(:,:), allocatable :: hamiltonian, test_eigvec
    real*8, dimension(:,:), allocatable :: test_outvec
    real*8, dimension(:),   allocatable :: test_eigval
  ...
  ! Allocate arrays
    ...
    allocate( test_eigval( dim_vecs) )
    allocate( test_eigvec( dim_vecs, dim_vecs ) )
#endif
...
#ifdef DEBUGHV
    ! Build hamiltonian
    open ( unit=94, file='det.list', status='old')
    read ( unit=94, fmt=9 ) ( determs(i), i=1, dim_vecs )
    close( unit=94 )
9 format(1x,I10)
    ...
    print *, "  ACTHV    .....   HV  "
    open(unit=96,file='acthv-debug',status='new')
    do i=1, num_vecs
!      print *, " ---------- ", i, " ------------- "
      do j=1, dim_vecs
        write(unit=96,fmt=13) vectors2(j,i), test_outvec(j,i)
      end do
    end do
13 format(1x,F10.7,F10.7)
    call diagonalize( hamiltonian, dim_vecs, test_eigvec, test_eigval )
    print *, "Eigenvalue 1:  ", test_eigval(1)
