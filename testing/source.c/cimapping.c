...
void convertrow2map(int *row, int n2v, struct rowmap *v2v)
{
	...
	if (row[i-1] == 1) {
	    csec->last = i - 1;
	    csec->next = (struct section *) malloc(sizeof(struct section));
	    csec->next->prev = csec;
	    csec = csec->next;
	    csec->next = NULL;
	    currstat = 0;
...
int generate_cimap(struct det *dlist, int ndets, int nactv, struct rowmap *hmap)
{
    ...
    int v;
    int ddiff;
    int naxc, nbxc, naxv, nbxv, naxcv, nbxv;
	...
	for (j = i, j < ndets; j++) {
		...
		ddiff = comparedets_ncas(
		    dlist[i], dlist[j], &naxc, &nbxc, &naxv, &nbxv, &naxcv,
		    &nbxcv, &axi, &axf, &bxi, &bxf, nactv);
	    } else {
		ddiff = comparedets_cas(
		    dlist[i], dlist[j], &naxc, &nbxc, &axi, &axf, &bxi, &bxf,
		    nactv);
	    }
	    if (ddiff > 2) {
		row[v] = 0;
