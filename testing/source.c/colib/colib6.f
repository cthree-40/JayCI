ccolib part=6 of 9.  logical expression evaluation routines.
cversion=4.1 last modified: 13-mar-91
cversion 5.0
c
c
...
c
         ...
         if ( itoken .ne. 0 ) then
            if ( fstrlen(line(lptr:)) .lt. fstrlen(nspctk(itoken)) )
     &           goto 904
            tkary(itkary) = nspcop( itoken )
            lptr = lptr + fstrlen( nspctk(itoken) )
c
         else
c
            olptr = lptr
