...
int pindex_double_rep_str(int *str, int io1, int fo1, int io2, int fo2, int ne)
{
	...
	if (new_loc < 0) {
		error_flag(mpi_proc_rank, new_loc, "pindex_double_rep_str: 1");
		return pindx;
	}
	str[new_loc] = fo1;
	...
	if (new_loc < 0) {
		error_flag(mpi_proc_rank, new_loc, "pindex_double_rep_str: 2");
		printf("error!\n");
...
int pindex_single_rep(int *str, int io, int fo, int lstr)
{
	...
	if (pos > lstr) {
		error_flag(mpi_proc_rank, pos, "pindex_single_rep");
		return 0;
	}
